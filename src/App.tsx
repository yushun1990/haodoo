...
import type { Book, Catalog } from './domain/book'
      ...
      <section className="detail-card">
        ...
        <div className="detail-card__body">
          ...
          <dl className="book-facts">
            ...
            {(book.volume?.track || book.volume?.title) && (
              <>
                <dt>卷册</dt>
                <dd>{[book.volume.track, book.volume.title].filter(Boolean).join(' · ')}</dd>
              </>
            )}
            {book.publishedAt && (
              <>
          ...
          <div className="resource-row">
            {book.epub && (
              <a href={book.epub.url} target="_blank" rel="noreferrer">
                横排 EPUB
              </a>
            )}
            {book.verticalEpub && (
              <a href={book.verticalEpub.url} target="_blank" rel="noreferrer">
                竖排 EPUB
              </a>
            )}
            {book.description && (
...
function CatalogPage({ catalog }: { catalog: Catalog }) {
  ...
  const filteredBooks = useMemo(() => {
    ...
    return catalog.books.filter((book) => {
      ...
      const haystack = [book.title, book.author, book.series?.name, book.volume?.title]
        .filter(Boolean)
        .join(' ')
        .toLocaleLowerCase()
  ...
  return (
    <main className="page">
      <header className="hero">
        ...
        <div className="hero__stat" title="当前 Classic 书目数量">
      ...
      <section className="catalog-tools" aria-label="书目检索">
        <label className="search-box">
          <span className="sr-only">搜索书名、作者或系列</span>
